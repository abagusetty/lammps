...
#if (DPCT_COMPATIBILITY_TEMP < 200)
...
#define THREAD_ID_X item_ct1.get_local_id(2)
#define THREAD_ID_Y item_ct1.get_local_id(1)
#define BLOCK_ID_X item_ct1.get_group(2)
#define BLOCK_ID_Y item_ct1.get_group(1)
#define BLOCK_SIZE_X item_ct1.get_local_range(2)
...
/*
DPCT1058:17: "atomicAdd" is not migrated because it is not called in the code.
*/
#define atom_add atomicAdd
#define ucl_inline static __inline__ 
...
#define CUDA_PRE_NINE
  ...
  ucl_inline double shfl_down(double var, unsigned int delta, int width,
    ...
    sycl::int2 tmp;
    /*
    DPCT1017:0: The  call is used instead of the __double2hiint call. These two
    ...
    */
    tmp.x() = dpct::cast_double_to_int(var);
    /*
    DPCT1017:1: The  call is used instead of the __double2loint call. These two
    calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    tmp.y() = dpct::cast_double_to_int(var, false);
    /*
    DPCT1023:2: The DPC++ sub-group does not support mask options for
    dpct::shift_sub_group_left.
    */
    tmp.x() = dpct::shift_sub_group_left(item_ct1.get_sub_group(), tmp.x(),
                                         delta, width);
    /*
    DPCT1023:3: The DPC++ sub-group does not support mask options for
    dpct::shift_sub_group_left.
    */
    tmp.y() = dpct::shift_sub_group_left(item_ct1.get_sub_group(), tmp.y(),
                                         delta, width);
    /*
    DPCT1017:4: The  call is used instead of the __hiloint2double call. These
    two calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    return dpct::cast_ints_to_double(tmp.x(), tmp.y());
  }
  ucl_inline double shfl_xor(double var, unsigned int lanemask, int width,
                             sycl::nd_item<3> item_ct1) {
    sycl::int2 tmp;
    /*
    DPCT1017:5: The  call is used instead of the __double2hiint call. These two
    calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    tmp.x() = dpct::cast_double_to_int(var);
    /*
    DPCT1017:6: The  call is used instead of the __double2loint call. These two
    calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    tmp.y() = dpct::cast_double_to_int(var, false);
    ...
    */
    tmp.x() = dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), tmp.x(),
                                             lanemask, width);
    /*
    DPCT1023:8: The DPC++ sub-group does not support mask options for
    dpct::permute_sub_group_by_xor.
    */
    tmp.y() = dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), tmp.y(),
                                             lanemask, width);
    /*
    DPCT1017:9: The  call is used instead of the __hiloint2double call. These
    two calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    return dpct::cast_ints_to_double(tmp.x(), tmp.y());
  ...
  ucl_inline double simd_broadcast_d(double var, unsigned int src, int width,
    ...
    sycl::int2 tmp;
    /*
    DPCT1017:10: The  call is used instead of the __double2hiint call. These two
    calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    tmp.x() = dpct::cast_double_to_int(var);
    /*
    DPCT1017:11: The  call is used instead of the __double2loint call. These two
    calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    tmp.y() = dpct::cast_double_to_int(var, false);
    ...
    */
    tmp.x() = dpct::select_from_sub_group(item_ct1.get_sub_group(), tmp.x(),
                                          src, width);
    /*
    DPCT1023:13: The DPC++ sub-group does not support mask options for
    dpct::select_from_sub_group.
    */
    tmp.y() = dpct::select_from_sub_group(item_ct1.get_sub_group(), tmp.y(),
                                          src, width);
    /*
    DPCT1017:14: The  call is used instead of the __hiloint2double call. These
    two calls do not provide exactly the same functionality. Check the potential
    precision and/or performance issues for the generated code.
    */
    return dpct::cast_ints_to_double(tmp.x(), tmp.y());
