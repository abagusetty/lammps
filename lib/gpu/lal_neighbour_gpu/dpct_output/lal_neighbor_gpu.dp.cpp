...
__kernel void calc_cell_id(const numtyp4 *restrict x_,
                           ...
                           int cells_in_cutoff, sycl::nd_item<3> item_ct1) {
...
__kernel void transpose(__global tagint *restrict out,
                        ...
                        int columns_in, int rows_in, sycl::nd_item<3> item_ct1,
                        sycl::accessor<tagint, 2, sycl::access_mode::read_write, sycl::access::target::local> block)
{
...
__kernel void calc_neigh_list_cell_v1(const __global numtyp4 *restrict x_,
				   const __global int *restrict cell_particle_id,
				   const __global int *restrict cell_counts,
				   __global int *nbor_list,
				   __global int *host_nbor_list,
				   ...
				   int ncellx, int ncelly, int ncellz,
				   int inum, int nt, int nall, int t_per_atom,
				   int cells_in_cutoff, sycl::nd_item<3> item_ct1, int *cell_list_sh,
				   numtyp4 *pos_sh)
{
  int tid = THREAD_ID_X;
