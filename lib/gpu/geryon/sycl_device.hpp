/***************************************************************************
  ...
  Utilities for dealing with SYCL devices
...
inline void ucl_flush(sycl::queue &cq) { cq.wait_and_throw(); }

inline void ucl_sync(sycl::queue &cq) { cq.wait(); }
...
class UCL_Device {
 public:
  ...
  inline sycl::queue & cq() { return cq(_default_cq); }

  /// Returns the queue indexed by i
  inline sycl::queue & cq(const int i) { return *(_cq[i]); }
  ...
  inline void push_command_queue() {
    _cq.push_back( new sycl::queue(_context, _sycl_device, sycl_asynchandler,
  ...
  { return _sycl_devices[i]->get_info<sycl::info::device::device::local_mem_size>(); }
  ...
  /// SYCL version supported by the device
  inline int sycl_device_version()
    { return sycl_device_version(_device); }
  /// SYCL version supported by the device
  inline int sycl_device_version(const int i)
    { return _properties[i].sycl_device_version; }
 ...
 private:
  ...
  std::vector<sycl::queue*> _cq;           // The default command queue for this device
...
void UCL_Device::clear() {
  ...
  for (int i=0; i<_sycl_devices.size(); i++)
    delete _sycl_devices[i];
  #endif
  _sycl_devices.clear();
...
int UCL_Device::set_platform(int pid) {
  ...
  auto const& devices = _sycl_platform.get_devices(sycl::info::device_type::all);
  _num_devices=devices.size();
  if (_num_devices == 0) {
    _num_devices=0;
    return UCL_ERROR;
  }

  sycl::device *device_list = new sycl::device[_num_devices];
  clGetDeviceIDs(_sycl_platform, CL_DEVICE_TYPE_ALL, n, device_list, &n);

  #ifndef GERYON_NUMA_FISSION
  // --- Store properties for each device
  for (int i=0; i<_num_devices; i++) {
    _sycl_devices.push_back( new devices[i] );
    add_properties( devices[i] );
  }
  #else
  ...
  for (int i=0; i<num_unpart; i++) {
    if (devices[i].get_info<sycl::info::device::partition_max_sub_devices>() > 1) {
      auto subdevice_list = devices[i].create_sub_devices<sycl::info::partition_property::partition_by_affinity_domain>(sycl::info::partition_affinity_domain::numa);
      for (const auto &sub_device : SubDevicesDomainNuma) {
	_sycl_devices.push_back( );
	add_properties( );
	_num_devices++;
      }
      
      for (uint j=0; j<num_subdevices; j++) {
        _sycl_devices.push_back(device_list[i]);
...
int UCL_Device::create_context() {
 ...
 void UCL_Device::add_properties(sycl::device& device_list) {
  ...
  op.work_item_size = device_list.get_info<sycl::info::device::max_work_item_sizes>();
  ...
  op.profiling_timer_resolution = device_list.get_info<sycl::info::device::profiling_timer_resolution>();

  op.ecc_support = device_list.get_info<sycl::info::device::error_correction_support>();
  ...
  op.max_sub_devices = device_list.get_info<sycl::info::device::device::partition_max_sub_devices>();
  ...
  op.sycl_device_version = cl_version_maj * 100 + cl_version_min * 10;
...
std::string UCL_Device::device_type_name(const int i) {
  if (_sycl_devices[i]->is_cpu())
    return "CPU";
  else if (_sycl_devices[i]->is_gpu())
    return "GPU";
  else if (_sycl_devices[i]->is_accelerator())
    return "ACCELERATOR";
  else
    return "DEFAULT";
}
