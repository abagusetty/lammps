...
class UCL_Kernel {
 public:
  ...
  inline void set_arg(const cl_uint index, const dtype * const arg) {
    clSetKernelArg(_kernel,index,sizeof(dtype),arg);
    if (index>_num_args) {
      _num_args=index;
      #ifdef UCL_DEBUG
  ...
  inline size_t max_subgroup_size(const size_t block_size_x) {
    ...
    clGetKernelSubGroupInfo(_kernel, _device,
...
inline int UCL_Kernel::set_function(UCL_Program &program, const char *function) {
  ...
  cl_uint nargs;
  clGetKernelInfo(_kernel,CL_KERNEL_NUM_ARGS,sizeof(cl_uint),&nargs,nullptr);
  _kernel_info_nargs=nargs;
  #ifdef NOT_TEST_CL_VERSION_1_2
  char tname[256];
  size_t ret;
  for (cl_uint i=0; i<nargs; i++) {
    clGetKernelArgInfo(_kernel,i,CL_KERNEL_ARG_TYPE_NAME,256,tname,&ret);
