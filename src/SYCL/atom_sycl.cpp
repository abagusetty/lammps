...
AtomSycl::AtomSycl(LAMMPS *lmp) : Atom(lmp)
{
...
AtomSycl::~AtomSycl()
{
  memoryKK->destroy_sycl(k_tag, tag);
  memoryKK->destroy_sycl(k_mask, mask);
  memoryKK->destroy_sycl(k_type, type);
  memoryKK->destroy_sycl(k_image, image);
  memoryKK->destroy_sycl(k_molecule, molecule);

  memoryKK->destroy_sycl(k_x, x);
  memoryKK->destroy_sycl(k_v, v);
  memoryKK->destroy_sycl(k_f, f);

  memoryKK->destroy_sycl(k_mass, mass);
  memoryKK->destroy_sycl(k_q, q);

  memoryKK->destroy_sycl(k_radius, radius);
  memoryKK->destroy_sycl(k_rmass, rmass);
  memoryKK->destroy_sycl(k_omega, omega);
  memoryKK->destroy_sycl(k_angmom, angmom);
  memoryKK->destroy_sycl(k_torque, torque);

  memoryKK->destroy_sycl(k_nspecial, nspecial);
  memoryKK->destroy_sycl(k_special, special);
  memoryKK->destroy_sycl(k_num_bond, num_bond);
  memoryKK->destroy_sycl(k_bond_type, bond_type);
  memoryKK->destroy_sycl(k_bond_atom, bond_atom);
  memoryKK->destroy_sycl(k_num_angle, num_angle);
  memoryKK->destroy_sycl(k_angle_type, angle_type);
  memoryKK->destroy_sycl(k_angle_atom1, angle_atom1);
  memoryKK->destroy_sycl(k_angle_atom2, angle_atom2);
  memoryKK->destroy_sycl(k_angle_atom3, angle_atom3);
  memoryKK->destroy_sycl(k_num_dihedral, num_dihedral);
  memoryKK->destroy_sycl(k_dihedral_type, dihedral_type);
  memoryKK->destroy_sycl(k_dihedral_atom1, dihedral_atom1);
  memoryKK->destroy_sycl(k_dihedral_atom2, dihedral_atom2);
  memoryKK->destroy_sycl(k_dihedral_atom3, dihedral_atom3);
  memoryKK->destroy_sycl(k_dihedral_atom4, dihedral_atom4);
  memoryKK->destroy_sycl(k_num_improper, num_improper);
  memoryKK->destroy_sycl(k_improper_type, improper_type);
  memoryKK->destroy_sycl(k_improper_atom1, improper_atom1);
  memoryKK->destroy_sycl(k_improper_atom2, improper_atom2);
  memoryKK->destroy_sycl(k_improper_atom3, improper_atom3);
  memoryKK->destroy_sycl(k_improper_atom4, improper_atom4);
...
void AtomSycl::sync(const ExecutionSpace space, unsigned int mask)
{
  if (space == Device && lmp->sycl->auto_sync) avecKK->modified(Host, mask);
...
void AtomSycl::modified(const ExecutionSpace space, unsigned int mask)
{
  avecKK->modified(space, mask);

  if (space == Device && lmp->sycl->auto_sync) avecKK->sync(Host, mask);
}

void AtomSycl::sync_overlapping_device(const ExecutionSpace space, unsigned int mask)
{
  avecKK->sync_overlapping_device(space, mask);
}
/* ---------------------------------------------------------------------- */

void AtomSycl::allocate_type_arrays()
{
...
void AtomSycl::grow(unsigned int mask)
{
  ...
  if (mask & SPECIAL_MASK) {
    ...
    memoryKK->grow_sycl(k_special, special, nmax, maxspecial, "atom:special");
    avec->grow_pointers();
    sync(Host, mask);
  }
}
...
int AtomSycl::add_custom(const char *name, int flag, int cols)
{
  int index;
  ...
  if (flag == 0 && cols == 0) {
    ...
    memoryKK->grow_sycl(k_dvector, dvector, ndvector, nmax, "atom:dvector");
...
void AtomSycl::remove_custom(int index, int flag, int cols)
{
  if (flag == 0 && cols == 0) {
...
void AtomSycl::deallocate_topology()
{
  memoryKK->destroy_sycl(k_bond_type, bond_type);
  memoryKK->destroy_sycl(k_bond_atom, bond_atom);
...
void AtomSycl::sync_modify(ExecutionSpace execution_space, unsigned int datamask_read,
...
AtomVec *AtomSycl::new_avec(const std::string &style, int trysuffix, int &sflag)
{
  ...
  if (!avec->syclable) error->all(FLERR, "SYCL package requires a sycl enabled atom_style");

  if (!hybrid_substyle_flag)
    avecKK = dynamic_cast<AtomVecSycl*>(avec);
