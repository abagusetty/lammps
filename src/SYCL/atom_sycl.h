...
#ifndef LMP_ATOM_SYCL_H
#define LMP_ATOM_SYCL_H
...
class AtomSycl : public Atom {
 public:
  ...
  class AtomVecSycl* avecKK;

  // map lookup function inlined for efficiency
  // return -1 if no map defined
  __attribute__((always_inline))
  static int map_sycl(tagint global, int map_style, DAT::tdual_int_1d k_map_array, dual_hash_type k_map_hash)
  {
    if (map_style == 1)
...
struct SortFunctor {
  ...
  ViewType source;
  Sycl::View<typename ViewType::non_const_data_type,typename ViewType::array_type,device_type> dest;
  IndexView index;
  SortFunctor(ViewType src, typename std::enable_if<ViewType::dynamic_rank==1,IndexView>::type ind):source(src),index(ind) {
    dest = Sycl::View<typename ViewType::non_const_data_type,typename ViewType::array_type,device_type>("",src.extent(0));
  }
  SortFunctor(ViewType src, typename std::enable_if<ViewType::dynamic_rank==2,IndexView>::type ind):source(src),index(ind) {
    dest = Sycl::View<typename ViewType::non_const_data_type,typename ViewType::array_type,device_type>("",src.extent(0),src.extent(1));
  }
  SortFunctor(ViewType src, typename std::enable_if<ViewType::dynamic_rank==3,IndexView>::type ind):source(src),index(ind) {
    dest = Sycl::View<typename ViewType::non_const_data_type,typename ViewType::array_type,device_type>("",src.extent(0),src.extent(1),src.extent(2));
  }
  SortFunctor(ViewType src, typename std::enable_if<ViewType::dynamic_rank==4,IndexView>::type ind):source(src),index(ind) {
    dest = Sycl::View<typename ViewType::non_const_data_type,typename ViewType::array_type,device_type>("",src.extent(0),src.extent(1),src.extent(2),src.extent(3));
  }
  __attribute__((always_inline))
