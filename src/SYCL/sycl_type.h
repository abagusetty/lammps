...
#if defined(SYCL_ENABLE_CUDA) || defined(SYCL_ENABLE_HIP) || defined(SYCL_ENABLE_SYCL) || defined(SYCL_ENABLE_OPENMPTARGET)
#define LMP_SYCL_GPU
#endif
...
// class KKDevice {
// public:
// #if defined(SYCL_ENABLE_CUDA) && defined(SYCL_ENABLE_CUDA_UVM)
...
struct MemsetZeroFunctor {
  ...
  void* ptr;
  SYCL_INLINE_FUNCTION void operator()(const int i) const {
    ((int*)ptr)[i] = 0;
  }
};
...
#ifdef LMP_SYCL_GPU
#define SNAP_SYCL_DEVICE_VECLEN 32
#else
...
struct alignas(2*sizeof(real_type_)) SNAComplex
{
  ...
  SYCL_FORCEINLINE_FUNCTION SNAComplex()
   : re(static_cast<real_type>(0.)), im(static_cast<real_type>(0.)) { ; }

  SYCL_FORCEINLINE_FUNCTION SNAComplex(real_type re)
   : re(re), im(static_cast<real_type>(0.)) { ; }

  SYCL_FORCEINLINE_FUNCTION SNAComplex(real_type re, real_type im)
   : re(re), im(im) { ; }

  SYCL_FORCEINLINE_FUNCTION SNAComplex(const SNAComplex& other)
   : re(other.re), im(other.im) { ; }

  SYCL_FORCEINLINE_FUNCTION SNAComplex& operator=(const SNAComplex& other) {
    re = other.re; im = other.im;
    return *this;
  }

  SYCL_FORCEINLINE_FUNCTION SNAComplex(SNAComplex&& other)
   : re(other.re), im(other.im) { ; }

  SYCL_FORCEINLINE_FUNCTION SNAComplex& operator=(SNAComplex&& other) {
    re = other.re; im = other.im;
    return *this;
  }

  SYCL_FORCEINLINE_FUNCTION SNAComplex operator+(SNAComplex const& other) {
...
SYCL_FORCEINLINE_FUNCTION SNAComplex<real_type> operator*(const real_type& r, const SNAComplex<real_type>& self) {
...
#define LAMMPS_LAMBDA SYCL_LAMBDA
